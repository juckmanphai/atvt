...
function toggleRecordsVisibility(pageType) {
    let detailsSection;
    let toggleButton;
    
    if (pageType === 'activity') {
        detailsSection = document.getElementById('activityDetailsSection');
        toggleButton = document.querySelector('#activity-page .button-container button');
    } else if (pageType === 'admin') {
        detailsSection = document.getElementById('adminDetailsSection');
        toggleButton = document.querySelector('#admin-page .button-container button');
    }
    
    if (detailsSection && toggleButton) {
        if (detailsSection.style.display === 'none' || detailsSection.style.display === '') {
            detailsSection.style.display = 'block';
            toggleButton.textContent = 'ซ่อนรายการ';
            toggleButton.style.backgroundColor = '#607d8b'; // สีเดิม
        } else {
            detailsSection.style.display = 'none';
            toggleButton.textContent = 'แสดงรายการ';
            toggleButton.style.backgroundColor = '#4caf50'; // สีเขียวเมื่อซ่อน
        }
    }
}

// ========== Initialize Records Visibility ==========
function initializeRecordsVisibility() {
    // ตั้งค่าเริ่มต้นให้ซ่อนรายการ
    const activitySection = document.getElementById('activityDetailsSection');
    const adminSection = document.getElementById('adminDetailsSection');
    
    if (activitySection) {
        activitySection.style.display = 'none';
        const activityButton = document.querySelector('#activity-page .button-container button');
        if (activityButton) {
            activityButton.textContent = 'แสดงรายการ';
            activityButton.style.backgroundColor = '#4caf50';
        }
    }
    
    if (adminSection) {
        adminSection.style.display = 'none';
        const adminButton = document.querySelector('#admin-page .button-container button');
        if (adminButton) {
            adminButton.textContent = 'แสดงรายการ';
...
function initializeApp() {
    ...
    if (activityTypes.length === 0) {
        ...
        initializeRecordsVisibility(); // ✅ เพิ่มบรรทัดนี้
